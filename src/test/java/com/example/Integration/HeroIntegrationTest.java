...
public class HeroIntegrationTest {
	...
	void testGetAll() throws Exception {
		Hero testHero = new Hero("Kazuha", "Anemo", "Sword", 90l);
		List<Hero> testHeroes = List.of(testHero);
		...
		//this.mvc.perform(get("/heroes/")).andExpect(status().isOk()).andExpect(content().json(testHeroesAsJSONArray));
	...
	void testUpdate() throws Exception{
		...
		Hero testSavedHero = new Hero(1l, "Ganyu", "Cryo", "Bow", 90l, team);
		String testSavedHeroAsJson = this.mapper.writeValueAsString(testSavedHero);

		RequestBuilder mockRequest = put("/heroes/update/1").content(testHeroAsJson)
				.contentType(MediaType.APPLICATION_JSON);
		//this.mvc.perform(mockRequest).andExpect(status().isOk()).andExpect(content().json(testSavedHeroAsJson)); //Only works when delete is not avaiable.
	}
